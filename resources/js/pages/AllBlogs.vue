    ...
    <div> 
        <v-container class="pt-0 all-blogs">
            <v-row no-gutters align="start">
                <v-col cols="3" class="w-lg-270px sticky-top">
                ...
                <v-col cols="9">
                    <div class="pt-5 ps-lg-7">
                        <v-row align="end" class="">
                            <v-col cols="12" sm class="pb-0 pt-3">
                                <div class="d-flex align-center">
                                    <div>
                                        <h1 class="fs-18" v-if="searchKeyword.length>0">{{ $t('search_results_for') }} "{{ searchKeyword  }}"</h1>
                                        <h1 class="fs-18" v-else-if="!is_empty_obj(currentCategory)">{{ currentCategory.name }}</h1>
                                        <h1 class="fs-18 ml-2" v-else>{{ $t('all_blogs') }}</h1>
                                    </div>
                        ...
                        <div class="mb-7">
                            <v-row class="row-cols-2 row-cols-sm-2 row-cols-md-3 row-cols-xl-3 md-gutters-10" v-if="blogs.length > 0">
                                <v-col
                                ...
                                >
                                    ...
                                    <v-card class="mx-auto my-6 mx-lg-2 border" style="box-shadow:none" v-else>
                                        <template slot="progress">
                                            <v-progress-linear color="deep-purple" height="10" indeterminate ></v-progress-linear>
                                        </template>

                                        <v-img height="250" :src="blog.banner"></v-img>
                        
                                        <v-card-title style="font-size:1rem" class="text-truncate-2 py-0 my-4 blog-title">{{ blog.title }}</v-card-title>
                            ...
                            <div class="pa-4 text-center fs-20" v-else>{{ $t('no_blog_found') }}</div>
                        </div>
                        <div class="text-center" v-if="totalPages > 1">
                            <v-pagination v-model="queryParamBlog.page" :length="totalPages" prev-icon="la-angle-left" next-icon="la-angle-right" :total-visible="7" elevation="0" @input="pageSwitch" class="my-4"></v-pagination>
                        </div>
                    </div>
                </v-col>
            </v-row>
...
<script> 
    ...
    async created() { 
        this.queryParamBlog.categorySlug = this.$route.params.categorySlug || this.queryParamBlog.categorySlug;
        this.queryParamBlog.searchKeyword = this.$route.params.searchKeyword || this.queryParamBlog.searchKeyword;  
        if (this.$route.query.page) this.queryParamBlog.page = this.$route.query.page; 
