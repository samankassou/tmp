            ...
            <v-form lazy-validation v-on:submit.prevent="rechargeWallet()" autocomplete="chrome-off">
                ...
                <v-row class="mb-4">
                    <v-col  cols="6" sm="4" md="3" v-for="(paymentMethod, i) in paymentMethods" :key="i" :class="[paymentMethod.status == 1 && paymentMethod.code != 'cash_on_delivery' ? '' : 'd-none']">
                        <label class="aiz-megabox d-block">
                            <input type="radio" name="wallet_payment_method" :checked="selectedPaymentMethod && paymentMethod.code == selectedPaymentMethod.code" @change="paymentSelected($event,paymentMethod)">
                ...
                <div v-if="selectedPaymentMethod && selectedPaymentMethod.code.includes('offline_payment')">
                    <v-text-field
                        ...
                        :error-messages="transactionIdErrors"
                        hide-details="auto"
                        required
                        outlined>
                    </v-text-field>
...
export default {
    ...
    methods:{
        ...
        async rechargeWallet(){
            ...
            if (this.selectedPaymentMethod && this.selectedPaymentMethod.code.includes('offline_payment') && this.$v.transactionId.$anyError) {
                return;
            }
            ...
            if(this.selectedPaymentMethod.code.includes('offline_payment')){
                ...
                await this.call_api("post", `payment/${this.selectedPaymentMethod.code}/pay`, formData,true); 
                this.snack({ message: this.$i18n.t("please_wait_for_approval"), color: "green" });
                ...
                this.$refs.makePayment.pay({
                    ...
                    expiration_month: this.months.indexOf(this.authorizeNet.expiration_month) + 1,
